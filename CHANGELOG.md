...
- **Contextual layers**: A layer can activate automatically for a selected Mac app, while manually held layers still take priority. Full-profile app links resolve first, conflicting cross-profile layer links are prevented, and command wheels can inherit the base wheel, define a layer-specific wheel, or be disabled for that layer.
...
- **Held outputs stay correct across every routing boundary**: Disabling mapping, changing profiles or app-activated layers, disconnecting a controller, and quitting ControllerKeys now release active keys, modifiers, stick directions, and MIDI controls that no longer apply. An unchanged held mapping remains active across app-layer transitions, so Shift and similar held actions do not blink off when focus changes.
