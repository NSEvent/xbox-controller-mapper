...
struct BeamdeskSurfaceMesh: Equatable, Sendable {
	let vertices: [SIMD3<Float>]
	let normals: [SIMD3<Float>]
	let indices: [Int32]
}

/// Immutable meshes shared by both SceneKit hand rigs.
///
/// Generating these signed-distance surfaces is CPU intensive. Build the
/// library once away from the main actor, then keep SceneKit object creation
/// on the main actor.
struct BeamdeskHandMeshLibrary: Sendable {
	let leftFist: BeamdeskSurfaceMesh
	let rightFist: BeamdeskSurfaceMesh
	let leftThumbBridge: BeamdeskSurfaceMesh
	let rightThumbBridge: BeamdeskSurfaceMesh
	let thumbCMC: BeamdeskSurfaceMesh
	let thumbMCP: BeamdeskSurfaceMesh
	let thumbIP: BeamdeskSurfaceMesh

	static func generate() -> BeamdeskHandMeshLibrary {
		BeamdeskHandMeshLibrary(
			leftFist: BeamdeskHandGeometry.fistMesh(inward: 1),
			rightFist: BeamdeskHandGeometry.fistMesh(inward: -1),
			leftThumbBridge: BeamdeskHandGeometry.thumbBridgeMesh(inward: 1),
			rightThumbBridge: BeamdeskHandGeometry.thumbBridgeMesh(inward: -1),
			thumbCMC: BeamdeskHandGeometry.thumbSegmentMesh(
				baseRadius: 0.15, tipRadius: 0.120, length: 0.42),
			thumbMCP: BeamdeskHandGeometry.thumbSegmentMesh(
				baseRadius: 0.152, tipRadius: 0.116, length: 0.44),
			thumbIP: BeamdeskHandGeometry.thumbSegmentMesh(
				baseRadius: 0.148, tipRadius: 0.104, length: 0.30)
		)
	}

	func fist(inward: Float) -> BeamdeskSurfaceMesh {
		inward > 0 ? leftFist : rightFist
	}

	func thumbBridge(inward: Float) -> BeamdeskSurfaceMesh {
		inward > 0 ? leftThumbBridge : rightThumbBridge
	}
}

enum BeamdeskBackgroundLoader {
	static func load<Value: Sendable>(
		_ build: @escaping @Sendable () -> Value
	) async -> Value {
		await Task.detached(priority: .userInitiated, operation: build).value
	}
}

enum BeamdeskHandMeshCache {
	private static let library = BeamdeskHandMeshLibrary.generate()

	static func load() async -> BeamdeskHandMeshLibrary {
		await BeamdeskBackgroundLoader.load { library }
	}
}
