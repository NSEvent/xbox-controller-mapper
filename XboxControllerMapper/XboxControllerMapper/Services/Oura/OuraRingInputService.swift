...
final class OuraRingInputService: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
	...
	private func probeTapToTagFeatureIfEnabled() {
		...
		// Round 3: arm tap pushes via the realtime command's feature BITMASK
		// (accelerometer = 0x20; rounds 1-2 showed the 2f family only reads
		// feature 07). Each frame keeps 0x20 set so the stream survives; the
		// stall watchdog re-arms plain 0x20 within 2s if the ring rejects one.
		let candidates: [[UInt8]] = [
			[0x06, 0x07, 0x21, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00],
			[0x06, 0x07, 0x22, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00],
			[0x06, 0x07, 0x24, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00],
			[0x06, 0x07, 0x28, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00],
			[0x06, 0x07, 0x27, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00]
		]
		for (index, frame) in candidates.enumerated() {
			DispatchQueue.main.asyncAfter(deadline: .now() + 2.5 * Double(index + 1)) { [weak self] in
