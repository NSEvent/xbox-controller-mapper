...
extension MappingEngine {
    ...
    nonisolated func handlePenOverlayPressed(_ button: ControllerButton, holdMode: Bool) {
        if UniversalControlMouseRelay.shared.sendUIEvent("penPress", button: button, holdMode: holdMode) {
            return
        }

        state.lock.withLock {
            state.penOverlayButton = button
            state.penOverlayHoldMode = holdMode
        }

        DispatchQueue.main.async {
            if holdMode {
                PenOverlayManager.shared.show()
            } else {
                PenOverlayManager.shared.toggle()
            }
        }
        inputLogService?.log(buttons: [button], type: .singlePress, action: "Pen")
    }

    nonisolated func handlePenOverlayReleased(_ button: ControllerButton) {
        if UniversalControlMouseRelay.shared.sendUIEvent("penRelease", button: button) {
            return
        }

        let (wasPenButton, wasHoldMode) = state.lock.withLock {
            let wasPenButton = state.penOverlayButton == button
            let wasHoldMode = state.penOverlayHoldMode
            if wasPenButton {
                state.penOverlayButton = nil
            }
            return (wasPenButton, wasHoldMode)
        }

        if wasPenButton && wasHoldMode {
            DispatchQueue.main.async {
    ...
    nonisolated func handlePenControlPressed(_ button: ControllerButton) -> Bool {
        guard PenOverlayManager.isControllerControl(button) else { return false }
        if UniversalControlMouseRelay.shared.remoteOverlayState().penVisible {
            return UniversalControlMouseRelay.shared.sendUIEvent("penControlPress", button: button)
        }
        guard PenOverlayManager.shared.threadSafeIsVisible else { return false }
        DispatchQueue.main.async {
    ...
    nonisolated func handlePenControlReleased(_ button: ControllerButton) -> Bool {
        guard PenOverlayManager.isControllerControl(button) else { return false }
        if UniversalControlMouseRelay.shared.remoteOverlayState().penVisible {
            return UniversalControlMouseRelay.shared.sendUIEvent("penControlRelease", button: button)
        }
        guard PenOverlayManager.shared.threadSafeIsVisible || button == .rightTrigger else { return false }
        DispatchQueue.main.async {
