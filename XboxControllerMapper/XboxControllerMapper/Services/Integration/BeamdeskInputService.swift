...
/// Tracks Beamdesk inputs until their matching wire release arrives.
struct BeamdeskInputState {
	...
	mutating func press(_ button: ControllerButton) -> Bool {
		pressedButtons.insert(button).inserted
	}

	mutating func release(_ button: ControllerButton) -> Bool {
...
final class BeamdeskInputService: NSObject, @unchecked Sendable {
	...
	private var inputState = BeamdeskInputState()
	...
	func stop() {
		...
		observing = false
		let buttons = inputState.releaseAll()
	...
	private func expireLinkIfStale() {
		...
		let buttons = expired ? inputState.releaseAll() : []
		if expired {
			gestureCooldown.reset()
			linkExpiryWorkItem = nil
		}
	...
	private func release(_ button: ControllerButton) {
		lock.lock()
		let changed = inputState.release(button)
		lock.unlock()
		guard changed else { return }
		controllerService.handleButton(button, pressed: false)
	}

	private func releaseButtons(_ buttons: Set<ControllerButton>) {
