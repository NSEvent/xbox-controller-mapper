...
- **レイヤー**：指定したボタンの長押しまたは切り替えで有効になる代替ボタンマッピングセットを作成
  - 1プロファイルにつき最大12個の追加レイヤー
  - レイヤーごとに、一時的な長押しまたは1回押す切り替えを選択
  - 未マッピングボタンのフォールスルー動作
...
このアプリは、キーボードとマウスの入力をシミュレートするために**アクセシビリティ権限**を必要とします。センシティブな権限だからこそ、アプリの完全なソースコードをPolyForm Noncommercial 1.0.0ライセンスで公開しています。

**このアプリが安全である理由：**

- **Source Available**：PolyForm Noncommercial 1.0.0ライセンスに基づき、非商用目的でアプリのソースコードを監査したり、自分でビルドしたりできます。寛容なオープンソースライセンスではありません。

- **仮名化されたオプトアウト可能な利用分析**：ランダムなインストールID、アプリ/ビルドとMacの情報、ロケール、インストール経路、試用/ライセンス状態のイベントを送信します。ライセンス有効化時にはGumroadの販売IDも送信されるため、そのインストールを購入記録と結び付けられますが、分析データベースには購入者の氏名やメールアドレスを保存しません。サーバーはおおよその国を判定し、生のIPアドレスではなくソルト付きハッシュのみを保存します。**設定 → 一般 → プライバシー**で無効にすると、これらのイベントとSparkleの週次システムプロファイル送信が止まります。更新確認とライセンス確認の通信は継続します。[プライバシー詳細](https://www.kevintang.xyz/apps/controller-keys/privacy-policy.html#analytics)

- **ローカル利用統計**：生のHIDレポート、入力したテキスト、マッピング内容は分析として送信されません。おすすめ機能とController Wrappedのため、ボタン/アクションの集計回数、移動距離、セッション合計をMac内に保存します。

- **署名と公証**：公式リリースはKevin TangのApple Developer IDで署名され、Appleの公証とGatekeeperの検証を受けます。これは発行者とリリースの完全性を確認するもので、公開ソースとのバイト単位の一致を証明するものではありません。

その他の通信は機能に応じて発生します。ライセンス確認はGumroad、更新・コミュニティプロファイル・コントローラーデータベースはControllerKeysのフィード/GitHubを利用します。ウェブサイトショートカットのfavicon取得や、ユーザーが設定したウェブフック、OBS、スクリプト、リンク、Mac間リレーも指定先へ接続します。
