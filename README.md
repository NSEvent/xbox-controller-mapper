...
- **Gyroscope Aiming & Gestures** (DualSense/DualShock 4):
  ...
  - Continuous gyro bias recentering while the controller rests keeps temperature drift from creeping the cursor (Steam Controller and DualShock 4)
...
- **Pseudonymous, Opt-Out Usage Analytics**: ControllerKeys sends a random install ID, app/build and Mac details, locale, install channel, trial/license lifecycle and conversion events, one-time setup milestones, and daily coarse usage aggregates. Those aggregates contain only an action-count range, broad feature categories, and whether a complex action was used. License activation also sends the Gumroad sale ID, making that install linkable to its purchase record; the analytics database does not store buyer names or email addresses. The server derives approximate country and stores a salted IP hash, not the raw IP. **Specific controller inputs, key codes, typed text, profile names or contents, app names, macro steps, scripts, quick text, URLs, controller identifiers, license keys, and configuration are never sent as analytics.** Turn this off in **Settings → General → Privacy**; this deletes pending events and also disables Sparkle's weekly system profile, while update and license checks still connect. See the [Privacy Policy](https://www.kevintang.xyz/apps/controller-keys/privacy-policy.html#analytics).
