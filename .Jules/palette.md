...
## 2024-10-24 - [SwiftUI Accessible Interactive Rows]
**Learning:** In SwiftUI lists, using `.onTapGesture` on an `HStack` (even with `.contentShape(Rectangle())`) lacks the built-in keyboard interactivity and accessibility traits of a standard button. This makes list items inaccessible for keyboard and screen reader users. However, when replacing this pattern with a `Button` wrapper and `.buttonStyle(.plain)`, it's crucial to retain `.contentShape(Rectangle())` inside the `Button` label. Without it, SwiftUI only treats the visible content (text/images) as the touch target, shrinking the clickable area and causing a UX regression for mouse/touch users.
**Action:** When converting interactive `onTapGesture` rows to `Button` elements for accessibility, always ensure the `.contentShape(Rectangle())` modifier is preserved inside the button label to maintain the full row touch target size.
