...
## 2024-08-01 - Avoid .onTapGesture for Interactive List Items
**Learning:** Found a pattern where interactive list rows (like scripts and macros) used `.onTapGesture` on standard views like `HStack`. This creates an interactivity trap that fails for keyboard navigation and is completely opaque to VoiceOver users.
**Action:** When a SwiftUI view element functions as a primary action (e.g., triggering edit mode), always wrap the content in a semantic `Button` with `.buttonStyle(.plain)` instead of attaching `.onTapGesture`. Ensure it is paired with a context-aware `.accessibilityLabel()`.
