...
## 2026-07-24 - [Replace .onTapGesture with Button]
**Learning:** Using `.onTapGesture` for list interactive elements causes accessibility and keyboard navigation issues since it lacks native button traits and keyboard support.
**Action:** Always wrap interactive list items in a `Button` (with `.buttonStyle(.plain)` or similar as needed) instead of applying `.onTapGesture` to ensure full accessibility and keyboard interactivity.
