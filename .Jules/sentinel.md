...
## 2026-06-27 - [Incomplete AppleScript Escaping]
**Vulnerability:** When executing AppleScript via `osascript`, untrusted input strings were only escaping backslashes and double quotes, leaving raw newlines (`\n`) and carriage returns (`\r`) unescaped.
**Learning:** Unescaped newlines can be parsed by the AppleScript compiler as statement terminators. This allows an attacker to break out of a string literal, resulting in command injection vulnerabilities (e.g., executing arbitrary shell commands via `do shell script`).
**Prevention:** When injecting untrusted strings into AppleScript templates, always escape raw newlines (`\n`) and carriage returns (`\r`) as well as backslashes (`\`) and double quotes (`"`).
