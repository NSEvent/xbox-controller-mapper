...
## 2026-06-26 - [Denial of Service via Pipe Deadlock (Runtime Executor)]
**Vulnerability:** The automation executor invoked `Process().waitUntilExit()` before fully reading the standard output and error pipes via `readDataToEndOfFile()`.
**Learning:** If a child process writes more data than the operating system's pipe buffer can hold (typically ~64KB), the child process will block waiting for the parent to read the data. If the parent is simultaneously blocked waiting for the child to exit (`waitUntilExit()`), a deadlock occurs, resulting in a Denial of Service. This pattern was found in core execution routines (`AutomationExecutor.swift`).
**Prevention:** Always ensure data is read from process pipes *before* or *concurrently* (using `readabilityHandler`) with waiting for the child process to exit. Wait until pipes are drained before blocking on process completion.
