...
## 2026-06-27 - [Denial of Service via Pipe Deadlock fixed in AutomationExecutor]
**Vulnerability:** The application executed `Process` shell commands and waited for them to exit (`process.waitUntilExit()`) before reading the standard output pipe.
**Learning:** If a child process writes more data than the OS pipe buffer can hold, it blocks writing while the parent blocks waiting for exit, resulting in a deadlock.
**Prevention:** Always drain pipe buffers (e.g., using `readDataToEndOfFile()`) *before* calling `process.waitUntilExit()`.
