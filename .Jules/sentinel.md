...
## 2026-08-14 - [Denial of Service via Log Flooding in Tests]
**Vulnerability:** Excessive synchronous `print` statements in high-frequency pathways like input processing loops and LED state synchronization were printing to `stdout` continuously.
**Learning:** High-frequency logging in macOS can fill up the system's log pipe buffers. During automated testing (like XCTest benchmarks or latency loops that simulate thousands of events per second), this causes the test runner or host process to hang and time out, resulting in a Denial of Service (DoS) of the test environment.
**Prevention:** Avoid putting unrestrained logging statements (such as "No PlayStation controller available" or "input ignored") in tight execution loops, particularly those that map to UI, HID input, or automated test pipelines, as they can cause system-level pipe stalls.
