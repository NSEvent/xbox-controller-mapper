...
## 2026-08-19 - [DoS/Deadlock Prevention via Process Pipe Handling]
**Vulnerability:** Use of `Process` combined with `waitUntilExit()` before fully reading output pipes (or using unread `Pipe()` assignments for unused streams) can cause an application to deadlock when the output exceeds the OS pipe buffer (~64KB).
**Learning:** Found instances where `Process()` was used and `pipe.fileHandleForReading.readDataToEndOfFile()` was called *after* `process.waitUntilExit()`. This is a classic deadlock pattern. Also found unread `Pipe()` instances assigned to `process.standardError`.
**Prevention:** Always read from pipes before or concurrently with `waitUntilExit()`. For unused output/error streams, assign them to `FileHandle.nullDevice` instead of instantiating an unused `Pipe()`.
