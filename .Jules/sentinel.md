...
## 2024-05-24 - [Denial of Service via Pipe Deadlock with File Descriptors]
**Vulnerability:** In an attempt to fix a pipe buffer deadlock by reading data using `readDataToEndOfFile()` before calling `waitUntilExit()`, test suites running shell processes hung indefinitely.
**Learning:** `readDataToEndOfFile()` is a blocking, synchronous call that waits until the pipe is closed. If a shell process spawns daemons or simply keeps file descriptors open after logically completing its main task (like `sleep` or background jobs), the pipe will remain open and `readDataToEndOfFile()` will block forever, preventing `waitUntilExit()` from ever being reached.
**Prevention:** To safely drain process output pipes without risking a 64KB deadlock *or* a file descriptor hang, use a non-blocking `readabilityHandler` to concurrently buffer output while waiting on `waitUntilExit()`, then set the handler to `nil` and perform a final read after the process has exited.
